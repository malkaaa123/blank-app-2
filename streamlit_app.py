...
# Criação de abas
tab1, tab2, tab3, tab4 = st.tabs(["Comparação de Índices", "Comentários", "Sentimentos", "Ficha Resumida"])
...
with tab1:
    if base_2023 is not None and base_2024 is not None:
        ...
        if gerencias_selecionadas and afirmativas_selecionadas and anos_selecionados:
            # Filtrar os dados pelas gerências e afirmativas selecionadas
            ...
            # Transpor as tabelas para colocar afirmativas na vertical
            ...
            # Combinar as tabelas transpostas
            ...
            # Tratar valores inválidos antes da conversão para inteiro
            comparacao.replace("**", 0, inplace=True)
            comparacao = comparacao.apply(pd.to_numeric, errors='coerce').fillna(0).astype(int)

            # Ordenar colunas para exibir gerências juntas por ano
            comparacao = comparacao[sorted(comparacao.columns, key=lambda x: (x.split()[0], x.split()[1]))]

            # Estilizar: Centralizar e pintar valores menores que 70 de vermelho
...
# Aba 4: Ficha Resumida
with tab4:
    if planilha_ficha is not None and base_2024 is not None and planilha_sentimentos is not None:
        st.subheader("Ficha Resumida")

        # Filtro para selecionar a Gerência
        ...
        if gerencia_selecionada_ficha:
            # Bloco de informações da planilha ficha
            ...
            if not ficha_info.empty:
                ...
                with col1:
                    st.markdown(f"**convidados:** {int(ficha_info['convidados'].iloc[0])}")
                    st.markdown(f"**Respondentes:** {int(ficha_info['Respondentes'].iloc[0])}")
                    adesao = formatar_adesao(ficha_info['Adesão'].iloc[0])
                    st.markdown(f"**Adesão:** {adesao:.2f}%" if adesao is not None else "**Adesão:** Dados indisponíveis")
                with col2:
                    st.markdown(f"**ENPS 23:** {int(ficha_info['ENPS 23'].iloc[0])}")
                    st.markdown(f"**ENPS 24:** {int(ficha_info['ENPS 24'].iloc[0])}")
                    st.markdown(f"**IVR 23:** {int(ficha_info['IVR 23'].iloc[0])}")
                    st.markdown(f"**IVR 24:** {int(ficha_info['IVR 24'].iloc[0])}")
                    st.markdown(f"**Feedback:** {int(ficha_info['Feedback'].iloc[0])}")
                    st.markdown(f"**Retenção:** {int(ficha_info['Retenção'].iloc[0])}%")

            # Bloco de afirmativas
            ...
            if not afirmativas_info.empty:
                ...
                for afirmativa in afirmativas_selecionadas:
                    if afirmativa in afirmativas_info.columns:
                        st.markdown(f"- **{afirmativa}:** {afirmativas_info[afirmativa].iloc[0]:.0f}")

                # Três maiores e menores pontuações
